...
class GestionnaireModulesDocker:
    ...
    def __formatter_configuration_container(self, container_name, config: dict = None, **kwargs):
        ...
        if nom_certificat is not None:
            self.__regenerer_certificat_container(nom_certificat)
            ...
            if path_certificat is not None:
                ...
                mounts.append({
                    ...
                    'source': path.join(MonitorConstantes.PATH_SOURCE_SECRET_DEFAUT, nom_certificat),
                    'type': 'bind'
                })
    ...
    def __regenerer_certificat_container(self, nom_certificat: str):
        # nom_certificat = config_image.get('certificat_compte')
        nom_role = nom_certificat.split('.')[1]
        path_secret_docker = path.join(MonitorConstantes.PATH_SOURCE_SECRET_DEFAUT, nom_certificat)
        os.makedirs(path_secret_docker, mode=0o755, exist_ok=True)

        # Le reparer est cree, la methode va automatiquement conserver le certificat/cle/CA
        self.__service_monitor.gestionnaire_certificats.generer_clecert_module(nom_role, self.__service_monitor.noeud_id)
        # self.__sauvegarder_certificat_container(nom_certificat, clecert)
